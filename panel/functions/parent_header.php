...
{    
  ...
  // include($_SERVER['DOCUMENT_ROOT'].'/'.$site_directory.'inc/login_function.php');
...
if (parent_Login::isLoggedIn()) {
    ...
    $useridme =  parent_Login::isLoggedIn();

        $username = "SELECT * FROM parents WHERE id='$useridme'";
        ...
        $user_name = mysqli_real_escape_string($conn,  $row['full_name']);
        ...
        //$user_rank = mysqli_real_escape_string($conn, $row['u_rank']);
         ...
         $GettingLoggedInAlerts = mysqli_query($conn, "SELECT * FROM user_alerts WHERE alert_to='$user_id' AND alert_totype='parent' ");
          $countingUnreadAlerts =  mysqli_num_rows($GettingLoggedInAlerts);

      //gettig messages 
          $RunMsg = mysqli_query($conn, "SELECT * FROM messages WHERE sent_to='$user_id' AND to_type='parent' limit 8");
                                    ...
                                    <li class="dropdown head-dpdn">
                                        ...
                                        while ($rowMsgs = mysqli_fetch_assoc($RunMsg)) {
                                            ...
                                            $MsgDate = mysqli_real_escape_string($conn, $rowMsgs['date']);
                                                echo '
                                            <li><a href="#">
                                               <div class="user_img"><img src="uploads/parents_profiles/'.$user_image.'" alt=""></div>
                                    ...
                                    <li class="dropdown profile_details_drop">
                                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                            <div class="profile_img">   
                                                <span class="prfil-img"><img src="uploads/parents_profiles/'.$user_image.'" alt=""> </span> 
                                                <div class="user-name">
                                                    <p>'.$user_name.'</p>
                                                    <span>Parent</span>
                                                </div>
