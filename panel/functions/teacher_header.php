...
include($_SERVER['DOCUMENT_ROOT'].'/edearth/inc/staff_login.php');
...
if ($TeacherLoginObj->isLoggedIn()) {
  ...
  //gettig messages 
          $RunMsg = mysqli_query($conn, "SELECT * FROM messages WHERE sent_to='$userid' AND to_type='teacher' AND read_or_unread='unread' ORDER BY 1 DESC LIMIT 5");
          $MsgCounter = mysqli_num_rows($RunMsg);
          
          
         //getting unread alerts
         $GettingLoggedInAlerts = mysqli_query($conn, "SELECT * FROM user_alerts WHERE alert_to='$userid' AND alert_totype='teacher' AND read_or_unread='unread' ORDER BY 1 DESC LIMIT 5");
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          ...
          <div class="dropdown-divider"></div>
          <form method="post" action="inc/teacher_sout.inc.php"> <li> <button class="btn btn-block btn-danger" name="confirm" type="submit" ><i class="fa fa-sign-out"></i> Logout</button> </li></form>
        </div>
      </li>
    </ul>
  </nav>
