  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">General Exam Grades</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
 ...
 <br><br>
   ...
   echo '
           ...
           <div
               ...
               <h4 class="text-center" style="color: #cccccc;background-color: #25476a;"> General Examination Results 
      ...
      ';
            ...
            //Querying from exams category
            $ExamCategory = mysqli_query($conn, "SELECT * FROM exams WHERE name='general'");
            $rowExamsCat = mysqli_fetch_assoc($ExamCategory);
            $ExamId = mysqli_real_escape_string($conn, $rowExamsCat['id']);
            //Getting created exam
            $CreatedExam = mysqli_query($conn, "SELECT * FROM exams_category WHERE exam_category='$ExamId' AND e_status='sent'");
                while($RowCReatedExam = mysqli_fetch_assoc($CreatedExam)){
            $CreatedExamId = mysqli_real_escape_string($conn, $RowCReatedExam['id']);
            $ExamCategoryName = mysqli_real_escape_string($conn, $RowCReatedExam['e_name']);
            $SentDate = mysqli_real_escape_string($conn, $RowCReatedExam['published_date']);

            $QuerryMarks = mysqli_query($conn, "SELECT * FROM exam_results WHERE marks_to='$user_id' AND exam_id='$CreatedExamId' ORDER BY 1 DESC LIMIT 15");
         ...
         while ($rowExamResults = mysqli_fetch_assoc($QuerryMarks)) {
            ...
            $Year = mysqli_real_escape_string($conn, htmlspecialchars($rowExamResults['e_year']));
