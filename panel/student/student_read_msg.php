    ...
    <div class="container" style="margin-top: 17px;">
        ...
        if (isset($_GET['id'])) {
          $Msg_id = mysqli_real_escape_string($conn, $_GET['id']);
              ...
              //gettig messages 
          $RunMsg = mysqli_query($conn, "SELECT * FROM messages WHERE id='$Msg_id' AND sent_to='$user_id' OR msg_from='$user_id' AND from_type='student' OR to_type='student' limit 1");
         ...
         if ($MsgCounter > 0) {
          ...
          while ($RowMsg = mysqli_fetch_assoc($RunMsg)) {
            ...
            $messageFrom = mysqli_real_escape_string($conn, $RowMsg['msg_from']);
             ...
             elseif ($fromtype == 'teacher') {
              $totype = 'teacher';
              $queryFrom = 'lecturers';
              $system_id = 'id';
              $profile_dir = 'teachers_profiles';
          }
          ...
          //Ending sender details
             ...
             if ($messageFrom == $user_id ) {
                echo'
                 ...
                 <div class="media border rounded-0" style="margin-bottom: 13px;background-color: #bed2b2;"><img class="img-fluid border rounded-circle mr-3" src="../uploads/'.$profile_dir.'/'.$senderProfile.'" style="width: 48px;">
...
$row_replies = $main->all_query_nolimit("messages_replys", 'LIMIT 10', 'msg_id',$Msg_id);


if ($row_replies[1] > 0) {

  foreach($row_replies[0] as $RowReplys) { 

  $messageBody =  $RowReplys['body'];
  $messageDate =  $RowReplys['date_sent'];
  $messageFrom =  $RowReplys['msg_from'];
   $messageTo =  $RowReplys['sent_to'];
  
  

   if ($messageFrom == $user_id && $RowReplys['from_type'] == 'student') {
