...
if (isset($_POST['proceed'])) {
	...
	if (empty($uid) || empty($pass)) {
		header("Location: ../login.php?signin=empty");
         ...
         $result = mysqli_query($conn, "SELECT * FROM librarianss WHERE username='$uid' OR email='$uid' OR nrc='$uid' ");
               ...
               if ($row = mysqli_fetch_assoc($result)) {
               	...
               	if ($hashedpwd == false) {
               		...
               		exit();
                        ...
                        $user_id = mysqli_query($conn, "SELECT id from librarianss where nrc='$uid' OR email='$uid' OR email='$uid'");
                        //getting user id
