...
function mailTemplate($code, $FullName){
      ...
      <tr> 
       <td align="center" style="-webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; mso-table-lspace: 0pt; mso-table-rspace: 0pt; border-collapse: collapse; display: table-cell;"> 
        <div class="text-xsmall textColorNormal" style="font-family: &quot;GT America Regular&quot;, &quot;Roboto&quot;, &quot;Helvetica&quot;, &quot;Arial&quot;, sans-serif; font-size: 11px; text-transform: uppercase; line-height: 22px; letter-spacing: 1px; color: rgb(79, 79, 101);">
         © 2020 Malcolm Moffat College Of Education.
        </div> 
        <div class="text-xsmall textColorNormal" style="font-family: &quot;GT America Regular&quot;, &quot;Roboto&quot;, &quot;Helvetica&quot;, &quot;Arial&quot;, sans-serif; font-size: 11px; text-transform: uppercase; line-height: 22px; letter-spacing: 1px; color: rgb(79, 79, 101);">
         All Rights Reserved
        </div> </td> 
...
if (isset($_POST['reset_pass_btn'])) {
  ...
  //verifying email
  $get_info = mysqli_query($conn, "SELECT * FROM students WHERE student_email='$email'");
  $searchTeachers = mysqli_query($conn, "SELECT * FROM lecturers WHERE email='$email'");
  $results_number = mysqli_num_rows($get_info);
  //getting user info
  

  if ($results_number>0) {
    $row_userByEmail = mysqli_fetch_assoc($get_info);
    $userId = mysqli_real_escape_string($conn, $row_userByEmail['id']);
    $userName = mysqli_real_escape_string($conn, $row_userByEmail['username']);
    $Fname = mysqli_real_escape_string($conn,$_POST['first_name']);
    $Lname = mysqli_real_escape_string($conn,$_POST['last_name']);
    $Full = $Fname .''. $Lname;
    $to_type = 'student';
    $htmlBody = mailTemplate($pin, $Full);
    //Inserting reset code
      $InsertToken = mysqli_query($conn, "INSERT INTO password_tokens(token, email, to_type) VALUES('$pin', '$email', '$to_type')");

       //sending email
          
    Mail::sendMail('Forgot Password! | Malcolm Moffat College Of Education', "".$htmlBody."", $email);
    header("location: ../reset_details.php?email=$email&status=non");
    //sending automatic
    // This function will return a random 
//Our custom function.
}
else{
    if(mysqli_num_rows($searchTeachers) > 0){
        $row_userByEmail = mysqli_fetch_assoc($searchTeachers);
          $userId = mysqli_real_escape_string($conn, $row_userByEmail['id']);
          $userName = mysqli_real_escape_string($conn, $row_userByEmail['username']);
          $to_type = 'teacher';
          $Fname = mysqli_real_escape_string($conn,$_POST['first_name']);
